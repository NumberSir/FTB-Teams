...
/// Properties should be declared statically by a mod (see [TeamProperties] as an example), and registered via
/// the [CollectTeamPropertiesEvent] event.
///
...
public abstract class TeamProperty<T> {
	...
	/// via [TeamPropertyType#register(Identifier, TeamPropertyType.FromNet)].
	///
	...
	/// Declare a property as sync'd to all (see [#shouldSyncToAll()]). By default, properties are not sync'd to
	...
	/// Deserialize the property's value from a string. This should act as the exact opposite of [#toString(T)],
	/// so that `prop.fromString(prop.toString(val)).get()` returns `val`.
	///
	/// @param string the string to parse
	/// @return an optional of the value, or `Optional.empty()` if the string could not be parsed
	...
	/// particular value of the property (see also [#writeValue(RegistryFriendlyByteBuf, T)]).
	///
	/// This method should _not_ write the property ID ([#getId()]).
	///
	/// @param buf the buffer
	public abstract void write(RegistryFriendlyByteBuf buf);

	/// Write the given value to a string, suitable for later parsing by [#fromString(String)]. This is
	/// _not_ the same as [#toString()] !
	///
